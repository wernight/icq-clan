...
void CResizingDialog::SetControlInfo(WORD CtrlId, WORD Anchore, float resize_x, float resize_y)
{
	if(Anchore == ANCHORE_LEFT)
		return; // Do nothing

	if (resize_x <= 0.0 || resize_x > 1.0)
		resize_x = 1.0;

	if (resize_y <= 0.0 || resize_y > 1.0)
		resize_y = 1.0;

	// Add resizing behaviour for the control
	DWORD c_info = CtrlId | (Anchore << 16);
	m_control_info.Add(c_info);
	m_control_info_resize.Add((int)(resize_x*10000) | ((int)(resize_y*10000) << 16));
}
...
void CResizingDialog::OnSize(UINT nType, int cx, int cy) 
{
	...
	if(m_old_cx)
	{
		...
		for(int i = 0; i < m_control_info.GetSize(); i++)
		{
			...
			c_info_res_x = LOWORD(m_control_info_resize[i]) / 10000.0;
			c_info_res_y = HIWORD(m_control_info_resize[i]) / 10000.0;
			...
			if ((Anchore & (RESIZE_HOR | ANCHORE_RIGHT)) == (RESIZE_HOR | ANCHORE_RIGHT))
			{
				WndRect.right += dx;
				WndRect.left += Round( dx * c_info_res_x );
			}
			else if(Anchore & RESIZE_HOR)
				WndRect.right += Round( dx * c_info_res_x );
			else if(Anchore & ANCHORE_RIGHT)
				WndRect.OffsetRect(dx,0);

			if ((Anchore & (RESIZE_VER | ANCHORE_BOTTOM)) == (RESIZE_VER | ANCHORE_BOTTOM))
			{
				WndRect.bottom += dy;
				WndRect.top += Round( dy * c_info_res_y );
			}
			else if(Anchore & RESIZE_VER)
				WndRect.bottom += Round( dy * c_info_res_y );
...
int CResizingDialog::Round(float f)
{
	if (f > 0)
		return (int) (f + 0.5);
	else
